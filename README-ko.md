...
<p align="center">
  Claude·ChatGPT·Gemini 한도에 <i>언제</i> 닿는지 안다 — 실시간 게이지·리셋 예측,
  그리고 어디서도 보여주지 않는 사용 이력. 확장 하나로, 모든 AI를, 개인도 팀도
</p>
...
- **Claude Code도 포함** — claude.ai·데스크톱 앱·Claude Code가 하나의 구독 사용량 풀을 공유하므로 CLI 소비도 같은 게이지에 잡힙니다. 다만 Anthropic이 그 풀을 단일 수치로만 노출해서, 표면별 분리가 아니라 합산 값입니다
...
- 주간 사용 리포트 (이메일)
- 모델별 토큰 사용량 추정
- 모델 스코프 주간 한도 추적 (Anthropic이 주간 버킷을 갖는 모델을 주기적으로 교체합니다)
- 429 제한 실시간 감지
</details>
...
1. **수집** — 확장이 claude.ai가 스스로 쓰는 것과 같은 계정 usage 엔드포인트, 그리고 ChatGPT·Gemini에서 사용량을 읽습니다 (대화 내용은 절대 수집하지 않음). Claude 구독 풀이 공유되므로 Claude Code·데스크톱 앱에서 쓴 양도 여기 포함됩니다
